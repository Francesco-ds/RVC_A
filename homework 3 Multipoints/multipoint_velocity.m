function [time,q,dq,ddq,dddq] = multipoint_velocity(qk,tk,st,dqi,dqf)
    ...
    n = size(qk,2); % number of pairs
    T = zeros(1,n-1); % time diff
    time = tk(1):st:tk(n);
    ...
    %euler approximation for velocity
    for k = 2:n
        v(k) = (qk(k)-qk(k-1))/(tk(k)-tk(k-1));
    end
    
    %compute velocity in each path point
    for k = 2: n-1
    ...
    % solution for the coefficient in the trajectory (slide 16)
    for k = 1:n-1
    ...
    for k=1:size(qk,2)-1
           ...
           %get the trajectories now that I have the coefficients
